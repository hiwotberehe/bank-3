...
# Optional dependencies — the app still runs even if these fail to install,
# it just quietly disables the related AI/PDF feature and shows a note instead.
try:
...
def ai_auto_words():
    ...
    if NUM2WORDS_AVAILABLE:
        ...
        words = _simple_number_to_words(amt)
    st.session_state["amount_words"] = words


def _simple_number_to_words(n: int) -> str:
    """Fallback number-to-words converter used only if num2words isn't installed."""
    ...
    if not branch_name or not full_name or not account_no or amount_number <= 0:
        ...
        with exp2:
            if FPDF_AVAILABLE:
                ...
                st.caption("PDF export unavailable — install `fpdf2` to enable this.")
        with exp3:
            if OPENPYXL_AVAILABLE:
                ...
                st.caption("Excel export unavailable — install `openpyxl` to enable this.")
